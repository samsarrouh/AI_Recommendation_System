...
def recommend():
    try:
        ...
        threshold_raw = data_json.get("threshold", 200.0)  # default threshold is 200
        ...
        # Optional: add 1 to each distance (if that's part of your logic)
        ...
        if not filtered_results:
            return jsonify({
                "message": "No relevant matches were found in the uploaded data for your query. Try refining your search or uploading more relevant documents.",
                "recommendations": [],
                "debug": {"modified_distances": distances, "threshold": threshold}
            })

        # Get the list of distances from filtered results
        distances_only = [item[0] for item in filtered_results]

        # Determine normalization boundaries:
        # Lower bound: use actual minimum if it's less than 120; otherwise, use 120.
        actual_min = min(distances_only)
        lower_bound = actual_min if actual_min < 120 else 120
        # Upper bound is always the user-provided threshold.
        upper_bound = threshold

        # Decide on the message (optional)
        ...
        for i, (distance, meta) in enumerate(filtered_results):
            ...
            # Compute normalized score for display (without filtering out low scores)
            if upper_bound == lower_bound:
                raw_score = 100
            else:
                normalized = 1 - ((distance - lower_bound) / (upper_bound - lower_bound))
                raw_score = 100 * normalized
            score = round(max(0, min(100, raw_score)), 2)

            # Keep keyword match filter only
...
def chat():
    try:
        data_json = request.get_json()
        user_query = data_json.get("query", "").strip()
        # Read chat threshold from payload; default to 270 if not provided.
        threshold_raw = data_json.get("threshold", 270.0)
        threshold = float(threshold_raw)
        ...
        if not filtered_pairs:
            ...
            return jsonify({"response": response_text, "debug": {"modified_distances": distances, "threshold": threshold}})
